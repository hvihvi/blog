...
And then, use `setTimeout` and `clearTimeout` to "kill" request that are no longer relevant as the user kept typing.
...
We can go a step further and abort unanswered requests via `AbortController`: https://dom.spec.whatwg.org/#aborting-ongoing-activities
